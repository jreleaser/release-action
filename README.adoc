...
A custom link:https://github.com/slsa-framework/[SLSA builder] is available for Zig projects. The following snippet shows
the minimum configuration to run the builder on GitHub Actions.
...
----
  release:
    permissions:
      ...
      actions: read
      packages: write
    uses: jreleaser/release-action/.github/workflows/builder_slsa3.yml@v1.0.0-zig
    with:
